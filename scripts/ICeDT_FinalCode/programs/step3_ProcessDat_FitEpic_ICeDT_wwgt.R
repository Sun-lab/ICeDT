...
#     - Original = Use the original weights (computed from non-rescaled data)
# If rescale = FALSE, then Weights = Original (see SECTION 5).
# Outputs from ICeD-T:
# fitnw -- no weight
# fitw0 -- use weight with arg "useIQR = 0"
# fitw2 -- (deprecated) -- use weight with arg "useIQR = 2"
...
if(Geneset=="Original"&&Weights=="Original"&&rescale==TRUE){
  ### Obtain the EPIC rescaled output, and reference profiles, use 
  ### the original variance weights. 
...
} else if(Geneset=="Original"&&Weights=="Revised"&&rescale==TRUE){
...
} else if(Geneset=="Revised"&&Weights=="Original"&&rescale==TRUE){
  load("./programs/EPIC-master/data/TRef.rda")
...
save(fitnw,fitw0,file=sprintf("./data/EPIC_ExpandedFits_GS%s_weights%s_rescale%s.RData",Geneset,Weights,rescale))
load(sprintf("./data/EPIC_ExpandedFits_GS%s_weights%s_rescale%s.RData",Geneset,Weights,rescale))
...
#|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||#
#-------------------------------------------------------------------#
figures_dir = sprintf("./figures/GS_%s_Wgt_%s_Rescale_%s/",Geneset,Weights,rescale)
dir.create(figures_dir, recursive = TRUE)

# for(j in c("nw","w0","w2")){  # fitw2 -- "useIQR = 2" -- is not available
for(j in c("nw","w0")){

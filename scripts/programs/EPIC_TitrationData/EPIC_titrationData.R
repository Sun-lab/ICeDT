...
fitnw_nT = ASICeD_fit(Y=(bulk+0.000001),fixedCT_rho=rep(0,ncol(bulk)),useRho = FALSE,RefMat = as.matrix(refProfiles+0.000001),
...
save(file="./data/EPIC_titrationData_fits.RData",fitnw_T,fitnw_nT,fitw0_T,fitw0_nT,fitw2_T,fitw2_nT)
load(file="./data/EPIC_titrationData_fits.RData")


fitTypes = c("nw_nT","nw_T","w0_nT","w0_T")
...
for(j in fitTypes){
  ...
  renorm4.recast = renorm4[,c("Bcells","CD4_Tcells","CD8_Tcells","NKcells","Dummy Tumor")]
  renorm4.recast[,5] = rowSums(renorm4[,c("Dummy Tumor","CAFs","Endothelial",'Macrophages')])
  
  ### Raw Comparison
  if(j%in%c("nw_nT","w0_nT")){
...
# EXTRAS                                                            #
#-------------------------------------------------------------------#
renorm4 = fitnw_nT$IC_Abundance
renorm4 = t(t(renorm4)/c(0.40,0.40,0.40,0.40,0.40,0.40,1.42,0.43))
renorm4 = renorm4/rowSums(renorm4)

renorm4 = fitw0_nT$IC_Abundance
renorm4 = t(t(renorm4)/c(0.40,0.40,0.40,0.40,0.40,0.40,1.42,0.43))
renorm4 = renorm4/rowSums(renorm4)

renorm4 = fitnw_T$IC_Abundance
renorm4 = t(t(renorm4)/c(0.40,0.40,0.40,0.40,0.40,0.40,1.42,0.43))
renorm4 = renorm4/rowSums(renorm4)

renorm4 = fitw0_T$IC_Abundance
renorm4 = t(t(renorm4)/c(0.40,0.40,0.40,0.40,0.40,0.40,1.42,0.43))
renorm4 = renorm4/rowSums(renorm4)
...
write.table(file="TRef_Output_Melanoma_orig.txt",x=TREF_out,quote = FALSE,row.names=FALSE,col.names = TRUE,sep="\t")
...
#-------------------------------------------------------------------#
### Full, LM22 Ref
CSORT_Full_LM22 = read.table(file = "./CIBERSORT.Output_FULL_lm22.csv",
...
### Rescale, TRef
CSORT_Resc_TRef = read.table(file = "CIBERSORT.Output_Rescale_TRef.csv",
                             sep=",",header=TRUE,row.names=1)
...
# Use the renorm4 section pertaining to w0_T, then run the two recast lines
# and fit
...
EPIC_Plot <- ggplot(data=EPIC_Plot_Dat,aes(x=Prop,y=EPIC,colour=CellGroup))+
             geom_point()+labs(y="EPIC",x="Flow Cytometry (Prop.)",colour="Cell Group")+
             theme(legend.position="none")

ICeDT_Plot <- ggplot(data=ICeDT_Plot_Dat,aes(x=Prop,y=ICeD_T,colour=CellGroup))+
              geom_point()+labs(y="ICeD-T (pY, wY)",x="Flow Cytometry (Prop.)",colour="Cell Group")+
              theme(legend.position="right")

lp <- get_legend(ICeDT_Plot)

Comb_Plot <- plot_grid(EPIC_Plot+theme(legend.position="none"),ICeDT_Plot+theme(legend.position="none"),NULL,lp,
                       labels=c("A","B",""),rel_widths=c(1,1,0.08,.3),nrow=1)

fnm = "./programs/EPIC_TitrationData/EPIC_ICeDT_MelVal_Plots.pdf"
